...
Since optimal matching was introduced by @Abbott-1986, it has been extensively used in social studies to answer questions about processes that take values in a categorical space and occur along a period of time. Particularly, in sociology there have been several studies following the professional career of groups of people (see, for instance, @Chan-1995, @Torsten-2014, @Gubler-2015).

The common objective of studies where optimal matching is used, is to find similarities and differences between the categorical sequences and obtain groups of trajectories that are similar by means of a clustering method applied to the matrix distance obtained with the algorithm.

In this work, we explore the possibility to obtain predictions of a secondary variable based on the information obtained from the sequential data via optimal matching using the implementation in the `R` package
<!-- TODO: add proper @ citation for TraMineR --> 
`TraMineR`.

<!-- TODO: use section labels: 
       bookdown.org/yihui/bookdown/cross-references.html -->
In Section 1, we describe the optimal matching algorithm, normalization options and the methods for obtaining the cost matrix. In Section 2, we introduce the data used in this work and perform optimal matching to obtain groups of sequences. In Section 3, we describe the method used for prediction with the data from the previous section. Finally, in section 4, we show some other scenarios that produce better predictions.
