...
colnames(experiments) <- c("Cost matrix",
                           "Normalization",
                           "Transition constant",
                           "NA cost",
                           "Min age",
...
```{r exp-results, out.width = "450px", fig.align = "center", fig.cap = "Relative MSE improvement in the prediction of personality traits."}
## To do Danielito:
## - Make legend title smaller
## - Decrease sapce between y-axis marks and plot(?)
## - Change color scale from white to some blue
...
```{r neuro3, out.width = "280px", fig.align = "center", fig.cap = "MSE of neuroticism prediction in scenario 3."}
## To do Danielito:
## - Increase size of axis marks
eval_experiments %>%
...
```{r neuro7, out.width = "280px", fig.align = "center", fig.cap = "MSE of neuroticism prediction in scenario 7."}
eval_experiments %>%
  tibble::add_column(Scenario = 1:13) %>%
  select(Scenario, result) %>%
  tidyr::unnest(result) %>%
...
```{r open, out.width = "280px", fig.align = "center", fig.cap = "MSE of openness prediction in scenario 13."}
eval_experiments %>%
  tibble::add_column(Scenario = 1:13) %>%
  select(Scenario, result) %>%
  tidyr::unnest(result) %>%
...
```{r extra, out.width = "280px", fig.align = "center", fig.cap = "MSE of extraversion prediction in scenario 9."}
eval_experiments %>%
  tibble::add_column(Scenario = 1:13) %>%
  select(Scenario, result) %>%
  tidyr::unnest(result) %>%
...
```{r cons, out.width = "280px", fig.align = "center", fig.cap = "MSE of conscientiousness prediction in scenario 2."}
eval_experiments %>%
  tibble::add_column(Scenario = 1:13) %>%
  select(Scenario, result) %>%
  tidyr::unnest(result) %>%
...
```{r agree, out.width = "280px", fig.align = "center", fig.cap = "MSE of agreeableness prediction in scenario 13."}
eval_experiments %>%
  tibble::add_column(Scenario = 1:13) %>%
  select(Scenario, result) %>%
  tidyr::unnest(result) %>%
...
```{r plot-seqdplot-fin, out.width = "400px", fig.align = "center", fig.cap = "Distribution of states for two clusters in Scenario 13."}
seqdplot(final$sd,
