...
Further, let $d = 2$ and let $K(i,j)$ be the cost associated to change from state $s_i$ to state $s_j$, for $i, j \in S$. This cost is given by:
...
where $P(s_i|s_j)$ is the estimated probability of transition from state $s_j$ in time $t$ to $s_i$ in time $t+1$ and $c$ is a constant, set to a value such that $0 \leq K(s_i, s_j) \leq 2$).
...
\par
Now, suppose we want to obtain the cost matrix with the method `TRATE` with $c=2$. We would procceed as follows:
...
\par
Table \@ref(tab:toy-cm) shows the full cost matrix obtained from transition probabilities, which is a symmetric matrix. As we can observe, the positions values displayed in $K[1,2], K[1,2], K[1,5], K[5,1]$ are the same as those obtained above.
...
\par
Let us remember that in `TraMineR` the constant is set to 2 by default. \@ref(tab:toy-cmod) shows the cost matrix obtained when we set the constant to 1.2, a value just above the maximum sum between the observed frequencies between states.
...
knitr::kable(
  ...
  align = "c",
  digits = 4,
  caption = "Cost matrix obtained from transition probabilities with $c=1.2$."
) %>%
  kableExtra::kable_styling(position = "center")
```
...
In addition to changing the value of $c$, we can opt for none or one of the normalization methods mentioned in the previous subsectionin order to obtain the distance matrix. Table \@ref(tab:toy-dists) shows some descriptive statistics of the obtained distances for different methods of normalization.
