...
Distance-based methods are statistical approaches that, instead of using raw data, use the similarity or dissimilarity between data points based on a distance metric. The main idea behind these methods is to obtain a distance matrix that allows the application of an unsupervised or supervised learning technique, such as different variations of clustering, dimensionality reduction, multidimensional scaling, and $k$-nearest neighbors ($k$-NN) in case we are interested in using the  data to predict other variable. Distance-based methods allow application with a variety of data types, in particular, categorical sequences, i.e. sequences that take values in a finite set of categories or states and are indexed by time.

Several ways to compute distances between categorical sequences have been proposed in the context of natural language processing and bioinformatics. Particularly, a class of measures, known as edit distances, provide a quantification of the dissimilarity of a pair of sequences by counting the minimum number of operations required to obtain a sequence from the other. For instance, @Hamming-1950 proposed a distance for sequences of the same length that counts the number of positions with different states. @Levenshtein-1966 generalized the Hamming distance to sequences of different lengths by considering the minimum number of single character edits required, namely insertion, deletion and substitution. The Damerau-Levenshtein distance allows for an additional operation known as transposition or swaping of characters (@Damerau-1964). The Jaro-Winkler similarity measure counts the number of matches and transpositions but does not fullfill the triangle inequality (@Winkler-1990). Additionally, there are several algorithms to find the longest common subsequence from a pair of subsequences (see @Bergroth-2000), this could be seen as a measure that allows for insertion and deletion but not substitution nor transposition. On the other hand, considering the distribution of the states in a sequence, for a pair of sequences, the euclidean or $\chi^2$ distance can be obtained. 

One generalization of the Levenshtein distance that allows for different substitution penalties for every pair of states is the Needleman-Wunsh algorithm that was developed by @Needleman-Wunsch-1970 with the aim of comparing biological sequences (for example, DNA or protein sequences). This algorithm is an application of dynamic programming, an iterative method that simplifies an optimization problem by breaking it into a recursion of smaller problems that are simpler to solve. By choosing the optimal operation at each step, it is guaranteed that the overall solution is optimal as well. An adaptation of this algorithm, known as optimal matching was introduced in social sciences by @Abbott-1986 and has been widely applied in sociology for life course or career path analysis. For instance, optimal matching has been employed in several studies tracking the professional development of specific groups of people, see @Chan-1995, @Torsten-2014 or @Gubler-2015, and to analyze life course data, for example, @Widmer-2009 or @Bastin-2015.

Critics of optimal matching point out has limitations. A notable flaw raised by @Wu-2000 lies in the definition of the cost matrix and the strong assumption about its symmetry as initially, the method considered substitution costs that were provided by an expert in the context. However, data-based aproaches for the calculation of the cost matrix have been proposed since then, see @Studer-2016.
...
We are interested in studying the effect of different hyperparameters  when using optimal matching to obtain pairwise distances of a group of sequences. Particularly, we consider the effects on both unsupervised and supervised learning techniques, namely clustering  and in the prediction of other variables of interest with $k$-NN. For this purpose we use a dataset that includes sequences with information about the romantic relationship of women and their scores in personality traits.
...
There are different choices for the distance function $d(\cdot, \cdot)$. For instance, the Euclidean o Mahalanobis distances are common choices but in our case we can obtain a dissimilarity matrix with OM.


## The OM algorithm {#om}

Optimal Matching is a technique widely applied in social sciences for the comparison of categorical sequences. For instance, in sociology, OM has been used for life course or career path analysis. OM uses the Needleman-Wunsch algorithm to identify similarities between biological sequences that are usually represented as strings of characters. 
...
where $\mathbf{P_.} = (P(s_1|s_.), \dots, P(s_n|s_.))'$ and $\alpha_l = \sum_{h=1}^n P(s_l|s_n)$.
