...
Optimal Matching (OM) is a technique widely applied in social sciences for the comparison of sequences that take values in a finite set of categories or states and are indexed by time. For instance, in sociology, OM has been used for life course or career path analysis. 

<!-- Any citation @ for Needleman-Wunsch? -->

OM uses the Needleman-Wunsch (@Needleman-Wunsch-1970) algorithm, that was developed to compare biological sequences. This algorithm is an application of dynamic programming, an iterative method that simplifies an optimization problem by breaking it into a recursion of smaller problems that are simpler to solve.
...
<!-- For completeness/clarity? Otherwise could be infinite? Also used at the end
of up to length $t_{max}$, with  $t$ \leq $t_{max}$.
-->

Now, let $X, Y \in \mathbf{S}$ be two sequences of size $t_X$ and $t_Y$, respectively. In order to numerically assess the similarities between the sequences $X$ and $Y$, we define an empty array $F$ of size $(t_X+1) \times (t_Y+1)$. Algorithm 1 below shows the initialization and recursion to fill the array $F$.
...
<!-- TODO: add proper @ citation for TraMineR -->
The `R` package `TraMineR` (@TraMineR) provides several functions to analyze and visualize sequential data. In particular, the package implements OM and offers several methods for computing the cost matrix $K$ and the normalization of the dissimilarity matrix.
...
- The Needleman-Wunsch algorithm with constant costs for mismatch is known as Levenshtein distance (@Levenshtein-1966).


### Example 

Suppose that $S$ is the alphabet and let $X = \{S,E,N,D\}$ and $Y = \{A,N,D\}$ be two sequences in $\mathbf{S}$. With $d = 2$ and
...
Given a set two sequences $X, Y \in \mathbf{S}$ of length $t_X$ and $t_Y$, respectively. Let $d(X,Y)$ be the distance between the sequences $X$ and $Y$, $t_{max}$ the length of the longest sequence in $\mathbf{S}$ and $d_{max}$ the maximum distance between any pair of sequences in $\mathbf{S}$.
