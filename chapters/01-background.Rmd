...
Distance-based methods are a class of statistical techniques that have in common the use of a distance measure between the observed data points to perform a specific task. The main idea behind these methods is to capture the structure of the data by obtaining a distance matrix. We encounter distance-based methods in both unsupervised and supervised learning. For example, different variations of clustering, dimensionality reduction and multidimensional scaling can be used to describe the data, or we can apply $k$-nearest neighbors ($k$-NN) when we are interested in using the data to predict other variable for which we have observed values.

Moreover, in case the available pair-wise distances are Euclidean, we can obtain centered inner products and this allows us to have several other techniques at our disposal, such as $k$-means clustering, principal component anlaysis (PCA), linear discriminant analysis and support vector machines (SVM), see @ESLII.
