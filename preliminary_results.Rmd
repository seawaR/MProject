...
- Manual corrections of several inconsistent and incomplete records (unify sequences with example data or exclude individuals).
- Several additional automatic checks to identify sequences with inconsistent data.
- Corrections based on secondary data source (e.g. double check number of children with another database that includes demographics and additional variables of the individuals).
- Identification and selection of the variables and patterns that provide a wider perspective of the individuals' situations (several scenarios considered).
...
- Prediction of personality scores based on the obtained distance matrix (k-nearest neighbors, distance-based LM) -> find the configuration that produces the best prediction
- Ideas?
