...
where $g_i$ is called the *indel* cost that takes values $g_i = 1/f_i$ for method `INDEL`, $g_i = \log[2/(1+f_i)]$ and $f_i$ is the relative frequency of the state $s_i$ for $i = 1, \dots, n$. 

**Note:** For methods `TRATE` and `FUTURE`, the unique *indel* value is $g = max_{1 \leq i,j \leq n} K(i,j)/2$, so that the cost of any transition is always lower or equal than deleting and inserting an element (or vice versa).
