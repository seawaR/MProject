...
Now, let $X, Y \in \mathbf{S}$ be two sequences of size $t_X$ and $t_Y$, respectively. We define an empty array, $F$, of size $(t_X+1) \times (t_Y+1)$ in order to assess the similarities between the sequences and obtain a numerical value associated with it.
...
Here, the value $d$ is the cost of inserting a gap in one of the sequences, and $k(y_{i-1}, x_{j-1})$ is the cost associated to change from the state $y_{i-1}$ to $x_{j-1}$, which is defined in a matrix $K$ of size $n \times n$, commonly known as the cost matrix.
...
In fact, when $F$ is completely filled, the value in the last cell, i.e. $F(t_X+1, t_Y+1)$ corresponds to the optimal cost of aligning the sequences $X$ and $Y$. It is possible to recover the steps that conduced to this a alignment with a traceback from the last cell. However, this is not necessary to perform OM.

The `R` package `TraMineR` provides several functions to analyze and visualize sequential data. In particular, the package implements OM and offers several methods for computing the cost matrix $K$ and the normalization of the dissimilarities matrix.
...
Using the `R` package `TraMineR` the cost matrix is calculated with transition rates between states. Base setup: `TRATE` and `maxlenght`
