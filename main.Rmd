...
\For{$i \gets 2,t_1+1$}
  \For{$j \gets 2,t_2+1$}
    \State $F(i,j) \gets max\{F(i-1, j)-d, F(i, j-1)-d, F(i-1, j-1)+k(x_{i-1}, y_{j-1}\}$
    \EndFor
\EndFor
\end{algorithmic}
...
There are different ways of calculating the distance between the new data point $x_{n+1}$ and the points in $\mathcal{D}$. For instance, the Euclidean o Mahalanobis distances are usually used. In our case we already count with a matrix distance obtained by OMA.
...
The following figure shows for every personality score and different values of $k$ the MSE, i.e. for $k = 1, \dots, 100$ we predict values of $Y$ and compare them with the observed values using the MSE.
